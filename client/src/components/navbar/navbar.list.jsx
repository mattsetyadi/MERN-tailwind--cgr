...
const NavbarList = ({history, logout, isAuth}) => {
    ...
    return (
        <ul className='font-bold flex-wrap flex md:mr-5 flex-col md:flex-row text-center'>
            ...
            {isAuth ? (
                <Button title='Logout' moreStyle='hover:text-primary' action={() => {
                    logout()
                }} />
            ) : (
                <>
                <Button title='Login' moreStyle='hover:text-primary' isButton={false} href='/login' />
                <Button title='Register' moreStyle='hover:text-primary' isButton={false} href='/register' />
                </>
            )}
